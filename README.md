...
## タイムカード機能
アプリホーム画面の「タイムカード」から利用できます。

### 設定手順
1. プロジェクト内の `google-apps-script.js` の内容をコピーします。
2. GoogleスプレッドシートのApps Scriptエディタを開きます（既存のプロジェクトに追加して構いません）。
3. 新しいスクリプトファイルを作成し、コードを貼り付けます。
4. デプロイ手順（ウェブアプリとして全員に公開）を行い、新しいURLを発行します。
   - ※既存のURLを使う場合も、新しいコードを含めて「新しいデプロイ」として更新する必要があります。
5. アプリの「設定」>「GAS WebアプリURL」にURLを入力して保存します。
...
### ⏱️ タイムカード機能
- **出退勤記録**: 顔認証用カメラ付きの出退勤打刻
- **同伴管理**: 出勤時に同伴の有無を選択可能
- **スプレッドシート連携**: タイムカード専用シートに自動記録
